...
public class MenuRenderer implements Disposable{
    private static final float darkness = 0.8f;
    ...
    private UnitType flyerType = Seq.with(UnitTypes.dagger, UnitTypes.toxopid).random();
    ...
    private void generate(){
        ...
        new Block[]{Blocks.grass, Blocks.grass},
        new Block[]{Blocks.basalt, Blocks.hotrock}
        );
        Block[] selected2 = Structs.random(
        new Block[]{Blocks.slag, Blocks.slag},
        new Block[]{Blocks.water, Blocks.water}
        );
    ...
    private void drawFlyers(){
        ...
        flyers((x, y) -> {
            Draw.rect(icon, x - 4f, y - 8f, flyerRot - 49);
        });

        float size = Math.max(icon.width, icon.height) * icon.scl() * 3f;
